        ...
        extends AbstractTestQueries
{
    ...
    private String ARRAY_CUM_SUM_SIGNATURE_UNSUPPORTED;
    ...
    private static final String KHLL_TIMESTAMP_TZ_TYPE_UNSUPPORTED_ERROR = ".*Failed to parse type \\[KHyperLogLog]. Type not registered.*";
    ...
    public void init(String storageFormat, boolean sidecarEnabled)
    {
        ...
        if (sidecarEnabled) {
            ARRAY_CUM_SUM_SIGNATURE_UNSUPPORTED = ".*Unexpected parameters \\(array\\(varchar\\)\\) for function native.default.array_cum_sum.*";
        }
        else {
            ARRAY_CUM_SUM_SIGNATURE_UNSUPPORTED = ".*Scalar function presto.default.array_cum_sum not registered with arguments.*";
        }
    }
    ...
    public void testArrayCumSumVarchar()
    {
        ...
        assertQueryFails(sql, ARRAY_CUM_SUM_SIGNATURE_UNSUPPORTED, true);
    ...
    */
    ...
    /// TODO: Fix result mismatch when sidecar disabled
    /// not equal
    /// Actual rows (84 of 84 extra rows shown, 89 rows in total):
    ///     [7773.41, null]
    ///     [2972.26, null]
    @Override
    @Test
    ...
    {
        if (sidecarEnabled) {
            super.testGenerateDomainFilters();
        }
    }
